...
with st.expander("S-Furnace", expanded=False):
    ...
    with col4:
        conc_feed = st.number_input("CONC. FEED RATE", value=mean_default('CONC. FEED RATE'))
...
pred_class = int(model.predict(scaled_in)[0])
...
if mode == "Counterfactual (GA)":
    ...
    def candidate_prob(candidate):
        ...
        return model.predict_proba(arr_scaled)[0]
        ...
        if run_sweep:
            ...
            if len(arrays) == 1:
                ...
                for v in vals:
                    ...
                    prob = model.predict_proba(scaler.transform(cand.reshape(1, -1)))[0]
                    pred = int(model.predict(scaler.transform(cand.reshape(1, -1)))[0])
